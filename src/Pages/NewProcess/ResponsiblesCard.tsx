...
type Props = {
  title: string,
  field_name: 'responsibles' | 'activePole' | 'passivePole',
  options?: PersonDataType[],
  refetch: () => void,
  loading: boolean,
}

const ResponsiblesCard: React.FC<Props> = ({ title, field_name, options, refetch, loading }) => (
  <Card>
      ...
      {(fields, { add, remove }) => {
        return (
          <div>
            ...
            <Button type="primary" onClick={() => add()}>Novo Responsável</Button>
          </div>
        )
      }}
    </Form.List>
